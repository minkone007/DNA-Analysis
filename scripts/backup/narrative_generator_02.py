...
=======================
Generates a bespoke plain-English genetics narrative report.
All text is written specifically for Minko's actual genotype combinations.
Run after the pipeline:
  python3 scripts/narrative_generator.py --person Minko
"""
...
def render(person, synthesis, results_dir, lang="en"):
    ...
    favourable = sum(1 for r in synthesis.values()
                     if any(w in (r.get("prediction","")).lower()
                            for w in ["favourable","below average","low risk"]))

    nav = "".join(
        f'<a href="#s_{sid}" class="nav-link">{stitle}</a>'
        for sid, stitle, keys in config["sections"]
        if any(k in synthesis for k in keys)
    )

    # Full HTML (use your original style block)
    html = f"""<!DOCTYPE html>
...
<style>
/* Paste your full original CSS here from the old render function */
...
/* ... (keep all your .nav-sidebar, .cover, etc. styles) ... */
</style>
</head>
<body>
...
<div class="main">
  ...
  {sections_html}
    ...
    print(f"✅ Saved Bulgarian version: {out.name}")
    return out
# ─────────────────────────────────────────────────────────────────────────────
# Entry points
# ─────────────────────────────────────────────────────────────────────────────
def generate(person_cfg, lang="en", verbose=True):
    ...
    if not synth_path.exists():
        if verbose: print(f"  WARN: synthesis.json not found for {name}")
        return None

    synthesis = json.load(open(synth_path))
    print(f"DEBUG: Generating {lang} version for {name}")
    out = render(name, synthesis, results_dir, lang)
    if verbose: 
        print(f"  OK   Narrative ({lang.upper()}) → {out.name}")
    return out
...
def main():
    ...
    parser.add_argument("--lang", choices=["en", "bg"], default="en", help="Language: en or bg")
